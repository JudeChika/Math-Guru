...
class FractionAdditionLogic {
  static FractionSolution? solve(List<String> fractionInputs) {
    try {
      ...
      // Add numerators
      int sumNumerator = convertedFractions.fold(0, (total, f) => total + f.numerator);
      String sumLatex = '\\frac{$sumNumerator}{$lcm}';

      workings.add(FractionWorkingStep(latexMath: '= $sumLatex'));
      explanations.add(FractionExplanation(
        description: 'Add the numerators and keep the denominator',
        latexMath: sumLatex,
      ));

      // Simplify
      Fraction simplified = FractionUtils.simplify(Fraction(sumNumerator, lcm));
