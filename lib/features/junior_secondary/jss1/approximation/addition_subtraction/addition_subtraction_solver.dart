...
class ApproximationAddSubtractLogic {
  /// Main solver for addition or subtraction.
  /// operation: "+" or "-"
  static ApproxResult solve({
    ...
    for (var i = 0; i < parsedInputs.length; i++) {
      ...
      if ((orig - rounded).abs() > 1e-9) {
        approxSteps.add(ApproxSolutionStep(
          latex: "${_removeTrailingZeros(orig)} approximately ${_removeTrailingZeros(rounded)}",
    ...
    // For fractions, combine as fractions; for decimals, just sum.
...
class _Fraction {
  ...
  _Fraction operator +(_Fraction other) {
    int lcm = denominator * other.denominator ~/ _gcd(denominator, other.denominator);
    int n1 = numerator * (lcm ~/ denominator);
    int n2 = other.numerator * (lcm ~/ other.denominator);
    return _Fraction(n1 + n2, lcm).simplified();
  }
